...
def precondition_by_tds(
  ...
  def update_fn(updates, state, params):
    ...
    count = state.count + jnp.array(1, dtype=jnp.int16)
    ...
    temp = jax.tree_map(lambda d, e:
                             tridiagonal_quic.tridiagKFAC(d,e,eps=eps),
                             nu_hat_d, nu_hat_e)
...
#     updates = jax.tree_map(lambda mf, a, b:
#                                 (mf[:-2]*a[:-1] + mf[1:-1]*b + mf[2:]*a[1:]),
#                                 mu_hat_flat, pre_e, pre_d)
#     updates = jax.tree_map(lambda u, m: u.reshape(m.T.shape).T if transpose else u.reshape(m.shape),
#                           updates, mu_hat)
...
def tds(learning_rate: ScalarOrSchedule, beta1=0.9, beta2=0.99, eps=1e-8, transpose=True):
    return combine.chain(
      precondition_by_tds(
          b1=beta1, b2=beta2, eps=eps, transpose=transpose),
