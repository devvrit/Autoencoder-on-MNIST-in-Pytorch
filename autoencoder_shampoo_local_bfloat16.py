...
def get_optimizer(opt, learning_rate):
  ...
  if opt=='distributed_shampoo' or True:
...
#Training
def train(state, model, train_inputs, test_inputs, rng, lrVec):
  ...
  p_train_step = jax.pmap(functools.partial(train_step, model=model),
                          axis_name='batch')
  ...
  for epoch in range(FLAGS.epochs):
    ...
    train_loss_val = p_eval_step(state=state,
                                 x=train_inputs.reshape(
                                     (jax.device_count(), -1, 784))).mean()
    test_loss_val = p_eval_step(state=state,
                                x=test_inputs.reshape(
                                    (jax.device_count(), -1, 784))).mean()
...
def main(argv):
  ...
  input_image_batch = input_image_batch.astype(float_type)
  ...
  model = Autoencoder(encoder_sizes, decoder_sizes, dtype=float_type,
