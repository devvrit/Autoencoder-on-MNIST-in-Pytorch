...
def rowwise_grafting(grads, diag_flat, bds_updates, transpose, graft_eps, beta2):
  
  diag = jax.tree_map(
        lambda df, g: df.reshape(g.T.shape).T  
        if transpose else df.reshape(g.shape), diag_flat, grads)
  if not transpose:
    grads = jax.tree_map(lambda g: g.T, grads)
    bds_updates = jax.tree_map(lambda u: u.T, bds_updates)
    diag = jax.tree_map(lambda d: d.T, diag)
  
  ## from here on grads, bds_updates and diag, are column major ordered
  normalized_grads = jax.tree_map(lambda g: g*(1/(jnp.linalg.norm(g,axis=0)+1e-16)),
                                 grads)
  diag = _update_moment(normalized_grads,diag,beta2,2)
  graft_updates = jax.tree_map(lambda g, d: g/(jnp.sqrt(d)+graft_eps),
                                 normalized_grads, diag)
  normalized_bds_updates = jax.tree_map(lambda u: u*(1/(jnp.linalg.norm(u,axis=0)+1e-16)),
                                 bds_updates)                              
  bds_updates = jax.tree_map(
          lambda u,gu:  u*jnp.linalg.norm(gu,axis=0),
           normalized_bds_updates,graft_updates)
  if not transpose:
    bds_updates = jax.tree_map(lambda u: u.T, bds_updates)
    diag = jax.tree_map(lambda d: d.T, diag)
  diag_flat = jax.tree_map(
        lambda d: d.T.reshape(-1) if transpose else d.reshape(-1), diag)
  return bds_updates, diag_flat
...
def precondition_by_bds(beta1: float = 0.9,
                        ...
                        innerIters = 15,
                        debias: bool = True,
                        weight_decay=0.05) -> optax.GradientTransformation:
  def init_fn(params):
    diag_flat = None
    if graft_type == 2:
      diag_flat = jax.tree_map(lambda g: jnp.zeros(len(g.reshape(-1)),
                                              dtype=g.dtype), params)
  ...
  def update_fn(updates, state, params):
    diag_flat = state.diag_flat
    grads = updates
    grads_flat = jax.tree_map(
        lambda g: g.T.reshape(-1) if transpose else g.reshape(-1), grads)
    mu = _update_moment(updates, state.mu, beta1, 1)

    count = state.count + jnp.array(1, dtype=jnp.int32)
    ## using a moving average lambda_t 
    lams = jax.tree_map( 
      lambda d,g :jnp.sqrt((beta2*jnp.sum(d)+(1-beta2)*jnp.square(jnp.linalg.norm(g)))/(1-beta2**count))+1e-16,
      state.nu_d, grads_flat )
    

    nu_e, nu_d = _update_nu_banded(grads_flat, state.nu_e,
    ...
    bds_flat_updates = jax.tree_map(lambda d, e, g:
    ...
    bds_updates = jax.tree_map(
        ...
        if transpose else mf.reshape(m.shape), bds_flat_updates, params)
    
    if graft_type == 2:
      # normalized_grads = jax.tree_map(lambda g: g/(jnp.linalg.norm(g)+1e-16),
      #                            grads_flat)
      # diag_flat = _update_moment(normalized_grads, diag_flat, beta2, 2)
      # graft_updates = jax.tree_map(lambda g, d: g/(jnp.sqrt(d)+graft_eps),
      #                            normalized_grads, diag_flat)
      # bds_flat_updates = jax.tree_map(
      #     lambda u, gu: (jnp.linalg.norm(gu) / (jnp.linalg.norm(u)+1e-16)) * u,
      #     bds_flat_updates, graft_updates)
      
      bds_updates, diag_flat = rowwise_grafting(grads, diag_flat, bds_updates, transpose, graft_eps, beta2)
    else:
      raise NotImplemented
    
    # # apply weight decay
    # bds_wd_updates = jax.tree_map(lambda u,p: u + weight_decay*p,bds_updates,params)
    # # momentum over bds updates
    # bds_wd_momentum_updates = jax.tree_map(lambda mu,u: mu*beta1+u,state.mu,bds_wd_updates)
    
    return bds_updates, PreconditionBandedDiagonalState(
        count=count, mu=mu, nu_e=nu_e, nu_d=nu_d, diag_flat=diag_flat)

  return optax.GradientTransformation(init_fn, update_fn)


def bds_rowwise_graft(learning_rate: ScalarOrSchedule,
        beta1: float = 0.9,
        beta2: float = 0.99,
        eps: float = 1e-8,
        graft_eps: float = 1e-8,
  ...
  return optax.chain(
      precondition_by_bds(beta1=beta1, beta2=beta2, eps=eps,
                          ...
                          transpose=transpose, weight_decay=weight_decay),
      optax.add_decayed_weights(weight_decay),
      scale_by_learning_rate(learning_rate),
  )
